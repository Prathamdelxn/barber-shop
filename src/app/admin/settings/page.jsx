...
//   const [shopId, setShopId] = useState("684ee7307f5718dd205b4308")
...
//     setIsSaving(true)
//     try {
//       const endpoint = shopExists ? `/api/shopinfo/fetch-by-id/${shopId}` : '/api/shop-settings'
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchShopSettings = async () => {
      try {
        // First try to fetch shop by owner ID (user._id)
        const response = await fetch(`/api/shopinfo/fetch-by-id/${user._id}`)
        ...
        // If no shop found, initialize with default values
        setSettings(prev => ({
          ...prev,
          shopName: `${user.name}'s Barber Shop`,
  ...
  const saveSettings = async () => {
    setIsSaving(true)
    try {
      const endpoint = shopExists ? `/api/shopinfo/update/${shopId}` : '/api/shopinfo/create'
      const method = shopExists ? 'PUT' : 'POST'

      // Include owner ID with the settings
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify(settingsWithOwner),
      })
        ...
        >
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {shopExists ? 'Manage your shop details' : 'Set up your new shop'}
            </p>
          </div>
          <div className="flex gap-2">
            <motion.button
